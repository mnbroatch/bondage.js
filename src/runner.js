...
class Runner {
  ...
  load(dialogue) {
    ...
    if (typeof dialogue === 'string') {
      ...
      const baselineWhitespace = lines.find((line) => { return line.trim() === '---'; })
        .match(/\s*/)[0];
      const trimmedDialogue = lines.map((line) => { return line.replace(baselineWhitespace, ''); })
        .join('\n');

      nodes = convertYarn(trimmedDialogue);
    } else {
      nodes = dialogue;
    }
